...
class CheckoutController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        if ($request->payment_option != null) {
            ...
            //  dd($request->session()->get('combined_order_id'));
                 //dd($product);
...
//        if (Session::has('cart') && count(Session::get('cart')) > 0) {
        if ($carts && count($carts) > 0) {
            ...
            // dd($categories);
            $billing_address = Billing_address::all();
            // dd($billing_address);
            return view('frontend.shipping_info', compact('categories', 'carts','billing_address'));
        }
    ...
    public function store_billing(Request $request){
        ...
        $billing -> country = $request->country_id;
        $billing -> state = $request->state_id;
        $billing -> city = $request->city_id;
        $billing -> pincode = $request->postal_code;
        $billing -> mobile = $request->phone;
